...
async def create_meal_plan(
    ...
    # Generate meal plan (pass start_date so day keys align with the requested start)
    meal_plan_data = generate_meal_plan(
        profile[0],
        plan_request.duration_days,
        plan_request.start_date,
    )
...
async def get_current_meal_plan(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Meal plan data is corrupted.")
...
async def customize_meal(
    ...
    # Update specific meal — convert validated MealData to plain dict
    if date_str in plan_data['meals']:
        plan_data['meals'][date_str][meal_update.meal_type] = meal_update.new_meal_data.model_dump(exclude_none=True)
    else:
